...
É importante frisar que as permissões sempre são referenciadas pelo seu `id`. Por exemplo: Se você desejar permitir que um certo item da tela só esteja visível para usuários com permissão `2`, você deverá atribuir o valor `2` à propriedade `visibleforpermissions` do item (na aba `items*`).
